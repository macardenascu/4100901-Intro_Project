...
int main(void)
{
  ...
  char *welcome_msg = "Iniciando programa: Bienvenido\r\n";
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 10000) // Cada 10000 ms
    {
    ...
    if (button_flag == 1)
{
  // 1. Encender ambos LEDs
  HAL_GPIO_WritePin(LED_EXT_GPIO_Port, LED_EXT_Pin, GPIO_PIN_SET); // Enciende LED externo
  HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);         // Enciende LD2


      // 2. Calcular cuándo apagar ambos LEDs (10 segundos desde ahora)
      led_ext_off_time = HAL_GetTick() + 10000;
    ...
    {
      // 1. Apagar ambos LEDs
      HAL_GPIO_WritePin(LED_EXT_GPIO_Port, LED_EXT_Pin, GPIO_PIN_RESET); // Apaga LED externo
